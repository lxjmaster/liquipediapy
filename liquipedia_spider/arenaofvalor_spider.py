...
class ArenaOfValorSpider(object):

    def __init__(self):

        self._config = Config()
        self.aov_obj = ArenaOfValor()
        self.mongo_obj = MongoObject()
        self.page_url = "https://liquipedia.net/arenaofvalor/"
    ...
    def get_teams(self) -> list:
        ...
        teams = self.aov_obj.get_teams()
        if len(teams) > 0:
            if self.config.has_section("MONGODB"):
                collection = self.mongo_obj.db["arenaofvalor_teams"]
                for team in teams:
    ...
    def get_team_info(self, team: dict) -> dict:
        ...
        team_info = self.aov_obj.get_team_info(team["name"])
    ...
    def get_players(self) -> list:
        ...
        players = self.aov_obj.get_players()
        if len(players) > 0:
            if self.config.has_section("MONGODB"):
                collection = self.mongo_obj.db["arenaofvalor_players"]
    ...
    def get_player_info(self, player: dict) -> dict:
        ...
        player_info = self.aov_obj.get_player_info(player["name"])
    ...
    def get_tournaments(self, tournament_type=None) -> list:
        ...
        tournaments = self.aov_obj.get_tournaments(tournament_type)
        if len(tournaments) > 0:
            if self.config.has_section("MONGODB"):
                collection = self.mongo_obj.db["arenaofvalor_tournaments"]
    ...
    def get_tournament_info(self, tournament: dict) -> dict:
        ...
        tournament_info = self.aov_obj.get_tournament_info(page)
    ...
    def main(self):
        ...
        #     input("111")
        tournaments = self.get_tournaments("A-Tier")
        for tournament in tournaments:
...
if __name__ == '__main__':
    arenaofvalor = ArenaOfValorSpider()
    arenaofvalor.main()
