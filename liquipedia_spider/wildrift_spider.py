...
class WildRiftSpider(object):

    def __init__(self):

        self._config = Config()
        self.lol_obj = WildRift()
        self.mongo_obj = MongoObject()
        self.page_url = "https://liquipedia.net/wildrift/"
    ...
    def get_teams(self) -> list:
        ...
        if len(teams) > 0:
            if self.config.has_section("MONGODB"):
                collection = self.mongo_obj.db["wildrift_teams"]
                for team in teams:
    ...
    def get_players(self) -> list:
        ...
        if len(players) > 0:
            if self.config.has_section("MONGODB"):
                collection = self.mongo_obj.db["wildrift_players"]
    ...
    def get_tournaments(self, tournament_type=None) -> list:
        ...
        if len(tournaments) > 0:
            if self.config.has_section("MONGODB"):
                collection = self.mongo_obj.db["wildrift_tournaments"]
